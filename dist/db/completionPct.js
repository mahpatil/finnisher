...
export function computeCompletionPct(threadId) {
    const db = getDb();
    const todosRow = db
        .select({
        total: count(),
        done: sql `COALESCE(SUM(CASE WHEN ${threadTodos.done} = 1 THEN 1 ELSE 0 END), 0)`,
    })
        .from(threadTodos)
    ...
    const recentRow = db
        .select({ n: count() })
        .from(sessions)
        .where(and(eq(sessions.threadId, threadId), gte(sessions.startedAt, new Date(Date.now() - SEVEN_DAYS_MS))))
        .get();
    ...
    const blockersRow = db
        .select({ n: count() })
        .from(blockers)
        .where(and(eq(blockers.threadId, threadId), ne(blockers.status, 'resolved')))
        .get();
...
//# sourceMappingURL=completionPct.js.map